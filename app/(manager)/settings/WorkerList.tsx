...
export default function WorkerList({ workers: initial, currentUserId }: { workers: Worker[]; currentUserId: string }) {
  const [workers, setWorkers] = useState(initial)
  const [editing, setEditing] = useState<string | null>(null)
  const [adding, setAdding] = useState(false)
  const [saving, setSaving] = useState(false)
  const [form, setForm] = useState({
    full_name: '', display_name: '', phone_number: '', role: 'worker',
  ...
  function openEdit(w: Worker) {
    ...
    setForm({
      ...
      role: w.role,
      specialty: (w.specialty || []).join(', '),
      daily_rate: w.daily_rate?.toString() || '',
      notes: w.notes || '',
      pin: '',
    })
  }
    ...
    <div className="px-4 py-5 space-y-3">
        ...
        <div key={w.id} className={`bg-white rounded-xl shadow-sm p-4 ${!w.is_active ? 'opacity-50' : ''}`}>
          <div className="flex items-start gap-3">
            <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 ${
              w.is_active ? (w.role === 'owner' ? 'bg-blue-600' : w.role === 'manager' ? 'bg-purple-500' : w.role === 'subcontractor' ? 'bg-amber-500' : 'bg-green-500') : 'bg-gray-300'
            }`}>
              {w.display_name[0]}
            </div>
            <div className="flex-1 min-w-0">
              ...
              {w.daily_rate && (
                <p className="text-xs text-green-600 font-bold mt-1">€{w.daily_rate}/ημέρα</p>
              )}
            </div>
            <div className="flex flex-col gap-1.5">
              <button onClick={() => openEdit(w)}
                className="text-xs bg-[#E8EEF8] text-[#0B2265] px-3 py-1.5 rounded-lg font-bold">
                ✏️
              </button>
              {w.id !== currentUserId && (
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-lg p-5 border-2 border-[#0B2265]">
          <h3 className="font-bold text-[#0B2265] text-base mb-4">
            {editing ? 'Επεξεργασία' : 'Νέος Εργάτης'}
          </h3>
          <div className="space-y-3">
            <div>
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Ονοματεπώνυμο *</label>
              <input value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))}
                className="w-full mt-1 px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="π.χ. Γεωργίου Δημήτρης" />
            </div>
